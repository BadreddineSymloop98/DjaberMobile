...
class AgentDraft {
  ...
  factory AgentDraft.fromJson(Map<String, dynamic> json) {
    // A disconnected page keeps its link (soft delete) but is not one the
    // agent answers on, nor one the form can show — so it is left out, rather
    // than counted as a ticked page the merchant cannot see.
    List<String> linked(Object? rows, String key, String nested) => [
          if (rows is List)
            for (final row in rows)
              if (row is Map<String, dynamic> && Json.mapOrNull(row[nested])?['isActive'] != false)

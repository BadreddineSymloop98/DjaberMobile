...
class _AgentDetailsScreenState extends State<AgentDetailsScreen> {
  ...
  /// The KPIs are live counts: coming back to the app reads them again.
  late final AppLifecycleListener _lifecycle = AppLifecycleListener(
    onResume: () {
      if (!_model.isFirstLoad) _model.refreshAfterInsight();
    },
  );
  ...
  List<Widget> _body(BuildContext context, L10n l10n) {
    ...
    return [
      ...
      if (metrics != null) ...[
        ...
        SizedBox(height: AppSpacing.xl),
      ] else if (_model.metricsError != null) ...[
        ApiErrorLine(error: _model.metricsError),
        Align(
          alignment: AlignmentDirectional.centerStart,
          child: OutlinedButton(onPressed: _model.reloadMetrics, child: Text(l10n.commonRetry)),
        ),
        SizedBox(height: AppSpacing.xl),
      ],
